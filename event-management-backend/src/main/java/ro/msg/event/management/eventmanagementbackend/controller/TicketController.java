...
public class TicketController {
    ...
    private final Converter<TicketView, TicketDto> convertToTicketDto;
    ...
    public ResponseEntity<TicketView> getFilteredTickets(@PathVariable Integer pageNumber, @RequestParam(required = false) String title, @RequestParam(required = false) String startDate, @RequestParam(required = false)String endDate){
       List<TicketView> ticketViews =  ticketService.getFilteredAndPaginated(title,startDate != null ? LocalDate.parse(startDate) : null, endDate != null ? LocalDate.parse(endDate) : null, pageNumber,TICKETS_PER_PAGE);
    ...
    public Integer getNumberOfPages(@RequestParam(required = false) String title, @RequestParam(required = false) String startDate, @RequestParam(required = false)String endDate){
        return ticketService.getNumberOfPages(title,startDate != null ? LocalDate.parse(startDate) : null,endDate != null ? LocalDate.parse(endDate) : null,TICKETS_PER_PAGE);
    }
}
